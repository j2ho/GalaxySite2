...
def read_site_db(search_metal=False):
    DB_path = '%s/ligands.1120'%SITE_DB_HOME
...
def read_pdb70_info(templ_s):
    templ_pdbs = []
    for templ in templ_s:
        templ_pdbs.append(templ.pdb_id)
    lines = open('%s/clu.list'%SITE_DB_HOME).readlines()
    clu = {}
    for ln in lines: 
        ln=ln.split()
        if ln[0] in templ_pdbs:
            for pdb_id in ln:
                clu[pdb_id]=ln
    return clu
